...
function checkHtml() {
    ...
    if (document.body.children.length !== 1) {
        console.error("document body has ".concat(document.body.children.length, " children, must have 1"));
        isGood = false;
    }
    ...
    if (scripts.length > 1 ||
        !((_a = scripts[0]) === null || _a === void 0 ? void 0 : _a.src.includes("vite/client"))) {
        console.error("only 1 <script> tag allowed in head for Vite client");
        isGood = false;
    }
    // no link tags other than a favicon link[rel=icon]
    if (document.querySelector("link:not([rel=icon])")) {
        console.error("only <link> allowed is 1 <link rel=\"icon\" ...> in head");
        isGood = false;
    }
    if (!isGood) {
        console.log("🛑 SimpleKit will not run with issues above. Fix HTML page hosting SimpleKit and/or disable browser plug-ins.");
    }
    return isGood;
}
function setupCanvas(colour) {
    if (colour === void 0) { colour = "whitesmoke"; }
    ...
    console.info(" created ".concat(canvas.width, " by ").concat(canvas.height, " canvas"));
    return canvas;
}
